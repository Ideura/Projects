...
export   ROMS_APPLICATION=YAEYAMA1
...
#export              FORT=pgi
#export       USE_OpenACC=on            # PGI Accarator fortran
...
#       export PATH=/opt/pgisoft/openmpi/bin:$PATH
        export PATH=/usr/apps.sp3/mpi/openmpi/1.6.5/p15.1/bin:$PATH
        export LD_LIBRARY_PATH=/usr/apps.sp3/mpi/openmpi/1.6.5/p15.1/lib:$LD_LIBRARY_PATH
      elif [ "${which_MPI}" = "mvapich2" ]; then
        export PATH=/usr/apps.sp3/mpi/mvapich2/2.0rc1/p14.6_cuda6.0/bin:$PATH
        export LD_LIBRARY_PATH=/usr/apps.sp3/mpi/mvapich2/2.0rc1/p14.6_cuda6.0/lib:$LD_LIBRARY_PATH
      fi
      ;;
    ...
    ifort )
      ...
      if [ -n "${USE_MPI:+1}" ]; then
        if [ "${which_MPI}" = "mpich" ]; then
          ...
          export      MCT_INCDIR=${HOME}/opt/intel/ompi/mct/include
          export      MCT_LIBDIR=${HOME}/opt/intel/ompi/mct/lib
      ...
      if [ -n "${USE_NETCDF4:+1}" ]; then
        if [ -n "${USE_PARALLEL_IO:+1}" ] && [ -n "${USE_MPI:+1}" ]; then
          if [ "${which_MPI}" = "mpich" ]; then
            ...
            export     NC_CONFIG=${HOME}/opt/intel/ompi/netcdf-4.3.3.1/bin/nc-config
            export NETCDF_INCDIR=${HOME}/opt/intel/ompi/netcdf-4.3.3.1/include
#            export     NC_CONFIG=/usr/apps.sp3/isv/amber/14_update12/i2013.1.046_ompi1.6.5_cuda7.0/amber14/bin/nc-config
#            export NETCDF_INCDIR=/usr/apps.sp3/isv/amber/14_update12/i2013.1.046_ompi1.6.5_cuda7.0/amber14/include
          fi
        else
          export       NC_CONFIG=${HOME}/opt/intel/ompi/netcdf-4.3.3.1/bin/nc-config
          export   NETCDF_INCDIR=${HOME}/opt/intel/ompi/netcdf-4.3.3.1/include
        ...
        export     NETCDF_INCDIR=${HOME}/opt/intel/netcdf-4.0.1/include
        export     NETCDF_LIBDIR=${HOME}/opt/intel/netcdf-4.0.1/lib
      fi
      ;;
    ...
    pgi )
      ...
      if [ -n "${USE_MPI:+1}" ]; then
        if [ "${which_MPI}" = "mpich" ]; then
          ...
          export      MCT_INCDIR=${HOME}/opt/pgi/ompi/mct/include
          export      MCT_LIBDIR=${HOME}/opt/pgi/ompi/mct/lib
          export  PARPACK_LIBDIR=/opt/pgisoft/openmpi/PARPACK
        elif [ "${which_MPI}" = "mvapich2" ]; then
          export        ESMF_DIR=/opt/gfortransoft/openmpi/esmf
          export      MCT_INCDIR=${HOME}/opt/pgi/mvapich2/mct/include
          export      MCT_LIBDIR=${HOME}/opt/pgi/mvapich2/mct/lib
      ...
      if [ -n "${USE_NETCDF4:+1}" ]; then
        if [ -n "${USE_PARALLEL_IO:+1}" ] && [ -n "${USE_MPI:+1}" ]; then
          if [ "${which_MPI}" = "mpich" ]; then
            ...
            export     NC_CONFIG=${HOME}/opt/pgi/netcdf-fortran-4.4.2/bin/nf-config
            export NETCDF_INCDIR=${HOME}/opt/pgi/netcdf-fortran-4.4.2/include
          elif [ "${which_MPI}" = "mvapich2" ]; then
            export     NC_CONFIG=${HOME}/opt/pgi/mvapich2/netcdf4/bin/nc-config
            export NETCDF_INCDIR=${HOME}/opt/pgi/mvapich2/netcdf4/include
          fi
        else
          export     NC_CONFIG=${HOME}/opt/pgi/netcdf-fortran-4.4.2/bin/nf-config
          export NETCDF_INCDIR=${HOME}/opt/pgi/netcdf-fortran-4.4.2/include
        ...
        export     NETCDF_INCDIR=${HOME}/opt/pgi/netcdf-4.0.1/include
        export     NETCDF_LIBDIR=${HOME}/opt/pgi/netcdf-4.0.1/lib
      fi
      ;;
    ...
    gfortran )
      ...
      if [ -n "${USE_MPI:+1}" ]; then
        if [ "${which_MPI}" = "mpich2" ]; then
          ...
          export      MCT_INCDIR=/usr/local/mpich2/mct/include
          export      MCT_LIBDIR=/usr/local/mpich2/mct/lib
          export  PARPACK_LIBDIR=/opt/gfortransoft/mpich2/PARPACK
        elif [ "${which_MPI}" = "openmpi" ]; then
          export        ESMF_DIR=/opt/gfortransoft/openmpi/esmf
          export      MCT_INCDIR=${HOME}/local/mct/gfortran-ompi/include
          export      MCT_LIBDIR=${HOME}/local/mct/gfortran-ompi/lib
          export  PARPACK_LIBDIR=/opt/gfortransoft/openmpi/PARPACK
        elif [ "${which_MPI}" = "mvapich2" ]; then
          export        ESMF_DIR=/opt/gfortransoft/openmpi/esmf
          export      MCT_INCDIR=${HOME}/opt/gnu/mvapich2/mct/include
          export      MCT_LIBDIR=${HOME}/opt/gnu/mvapich2/mct/lib
          export  PARPACK_LIBDIR=/opt/gfortransoft/openmpi/PARPACK
        fi
      fi
      ...
      if [ -n "${USE_NETCDF4:+1}" ]; then
        if [ -n "${USE_PARALLEL_IO:+1}" ] && [ -n "${USE_MPI:+1}" ]; then
          if [ "${which_MPI}" = "mpich2" ]; then
            ...
            export     NC_CONFIG=${HOME}/opt/gnu/mvapich2/netcdf4/bin/nc-config
            export NETCDF_INCDIR=${HOME}/opt/gnu/mvapich2/netcdf4/include
          fi
        else
          export       NC_CONFIG=${HOME}/opt/gnu/netcdf4/bin/nc-config
          export   NETCDF_INCDIR=${HOME}/opt/gnu/netcdf4/include
        fi
      else
        export     NETCDF_INCDIR=${HOME}/opt/gnu/netcdf3/include
        export     NETCDF_LIBDIR=${HOME}/opt/gnu/netcdf3/lib
      fi
      ;;
